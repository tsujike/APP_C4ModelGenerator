...
/**
 * post-v1.0(FR-8): 「履歴」メニューに残すファイルの最大件数。古いものから溢れさせる。
 * ブラウザはファイルの絶対パスをJavaScriptへ渡さないため、履歴に表示できるのはファイル名と
 * 最終利用日時だけであり、件数を増やしても同名ファイルの区別が付かず選びにくくなる。
 * 10件は「直近の作業ファイルを1クリックで戻せる」という目的に対する単純な上限値。
 */
export const FILE_HISTORY_MAX = 10;
