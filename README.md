...
- マーカーは**その行だけ**で書きます。`%%L1: 名前` や `%%L1 詳細` はマーカーとして扱われず通常のコメントになります(FR-7.2)。この場合は警告が出て、そのレベルの図は登録されません。
...
- 同じレベルのマーカーが複数あるときは**最初の1つが勝ち**、2つ目以降は警告にして無視されます。最初のマーカーより前の非空行、および本文が空のマーカーも警告になります。
...
`%% MODE: zoom` のように書くと、L1→L8という軸そのものが何を意味するかを宣言できます。ソースの先頭に1行置くのが標準です(最初のレベルマーカーより前に書いても「マーカーより前の行」警告は出ません)。
