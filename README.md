...
- ソースはブラウザのlocalStorageに自動保存され、次回起動時に復元されます。保存済みソースが無い初回起動時は、組込サンプル「インターネットバンキング」が既定で表示されます(サンプルをメニューから選ぶ機能はpost-v1.0で廃止しました)。
...
軸の意味(`%% MODE:`)と各レベルの名前(frontmatterの `title:`)の組み合わせを、このアプリでは「プロファイル」と呼びます。Mermariumにプロファイルを登録・保存する機能はなく、あくまで上記の書き方の規約です。C4モードは、事実上「軸はズーム(詳細度)、レベル名はContext/Containers/Components/Code」という1つのC4プロファイルだと捉えられます。C4以外の題材(採用プロセス)に `MODE: views` とfrontmatter titleを組み合わせた見本は `src/samples/views-profile.ts` にありますが、サンプルをメニューから読み込む機能は廃止したため、試すにはこのファイルの内容をエディタへ貼り付けてください。
...
VSCode等の外部エディタで編集しているファイルを、保存するたびに自動で画面へ反映させる機能です。旧来の「保存」(ソースを`.txt`でダウンロード)「開く」(その場限りの1回読み込み)ボタンはpost-v1.0で廃止し、「ファイルを選択」ボタンと「履歴」メニューへ一本化しました。上記の「概要」に書いたSVG/PNGエクスポートはこれとは別機能で、廃止していません。

想定ワークフロー:

1. ツールバーの「ファイル:」グループにある**「ファイルを選択」**ボタンを押します。
2. **Chrome / Edge**(File System Access API対応)では、ネイティブのファイル選択ダイアログが開きます。VSCodeで編集したい`.txt`/`.md`/`.mmd`/`.mermaid`ファイル(「すべてのファイル」からも選択可)を選ぶと、現在のソースを破棄してよいかの確認を経てそのファイルへ「リンク」します。以後はアプリ側が1秒間隔でファイルの更新を確認しており、VSCode側で**保存**するとすぐに画面の図が自動で再解析・再描画されます(タブがバックグラウンドの間は確認を止めます)。リンク中はエディタが**読み取り専用**になり、直接編集できません(編集は常にVSCode側で。アプリ側で編集しても、次に外部ファイルが保存された時点で上書きされて消えてしまうため)。
   **Firefox / Safari**(File System Access API非対応)では、代わりに従来どおりの`<input type="file">`が開き、選んだファイルをその場限りで1回だけ読み込みます。ボタン自体は無効化されませんが、以後の自動反映・履歴登録は行われません。
3. 選んだファイルの内容がエディタに読み込まれ、タイトルもファイル名(拡張子除く)になります。
4. リンク中に別のファイルへ切り替えたい場合も、もう一度**「ファイルを選択」**を押すだけです(トグルボタンではなく、押すたびに新しいファイルへ乗り換えられます)。
5. Chrome / Edgeでは、これまでにリンクしたファイルが**「履歴」**メニューに最大10件、最終利用日時が新しい順に並びます。選ぶと1クリックで再リンクできます。**履歴に表示できるのはファイル名と最終利用日時だけで、フルパス(`D:\...\foo.txt`等)は表示できません** — File System Access APIがファイルの絶対パスをブラウザからJavaScriptへ一切渡さない仕様のためです。
6. 終了するときは**「リンク解除」**ボタン(リンク中だけ表示されます)を押します。エディタが編集可能な状態に戻りますが、履歴からは消えないので、次回もそのファイルを「履歴」から選び直せます。

書き戻し(アプリ→ファイル)は行いません。あくまでファイル→アプリの一方向の反映です。ファイルが見つからない・読み込めなくなった場合は自動的にリンクが解除され、履歴からも取り除かれます。
...
サンプルの完全な入力例は `src/samples/internet-banking.ts` / `src/samples/ec-site.ts` を参照してください。「インターネットバンキング」は初回起動時(localStorageに保存済みソースが無いとき)の既定表示として引き続き使われますが、サンプルをメニューから選んで読み込む機能自体はpost-v1.0で廃止しました。
