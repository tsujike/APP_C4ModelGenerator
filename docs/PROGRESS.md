...
## 2026-07-31 FR-7.11/FR-7.5 の誤検知修正(Kennyの実文書での報告)

### 何が起きたか

前日に入れたFR-7.11の警告が、Kennyの実文書で**全件誤検知**した。報告されたIssuesパネルの中身:

```
[警告] L2/L3/L4: 最初のレベルマーカー(%%L1 など)より前に書かれた行は…無視します。
[警告] L2/L3/L4/L81: %%Ln の後ろに文字が続くため、この行はレベルマーカーとして扱いません。…
```

原因になった書き方(Kennyの文書の冒頭。これは普通の使い方であって間違いではない):

```
%% MODE: views
%% L1=宣言(あるべき姿) / L2=実態(いま動いているもの) / L3=令9期の是正経緯(…)
%% L1とL2は箱を揃えてある — 差分は「線」と「⚠」だけ。重ねて読めるようにするため
%% L3は主題が違うため色の意味が変わる。各ノード先頭の【 】が主語（どちらの帳簿の話か）
```

2つの別々の不具合が重なっていた。

1. **FR-7.11(`findNamedMarkerLikeLines`)**: 行の見た目だけで判定していたため、`%% L1=宣言…`
   `%% L1とL2は…` `%% L3は主題が…` のような**ただのコメント**を「名前付きマーカーもどき」と
   誤認した。`%%L1`〜`%%L3` の正規マーカーは別行にきちんと書かれており、何も壊れていない。
   レベル本文中のコメント(`%% L3の凡例: 赤=是正前`)も同様に誤検知した(報告の`L81`がこれ)。
2. **FR-7.5(`splitMermaidLevels`)**: 最初のマーカーより前の非空行を一律に警告していたため、
   冒頭の凡例コメント3行が「無視します」と警告された。MODE行だけは対象外にしてあったが、
   コメント行一般が漏れていた。

### 直した内容

- `findNamedMarkerLikeLines` は、**その行が指すレベルに正規マーカーが1行も無いときだけ**報告する。
  この警告に意味があるのは「名前を付けたせいでそのレベルが登録されなかった」ときだけで、
  登録済みレベルを指す行は何も壊していないため見送る。
- `splitMermaidLevels` の「マーカーより前の非空行」警告は、**`%%` コメント行を対象外**にした。
  MODE行の個別扱いはこの一般規則に吸収された(MODE行の分岐は残っているが、下の分岐でも拾われる)。
- 要件定義書 FR-7.5 / FR-7.11、設計書 §13.5、README(レベルマーカーの規則)に条件を明記。

### 検証結果(客観)

- `tsc=0 test=0 lint=0 build=0`、vitest **24ファイル / 301テスト pass**(誤検知回避2件・真陽性1件を追加)。
- ブラウザ実測(`/tmp/verify_fp.js`、`PAGE_ERRORS=[]`):
  Kennyの文書と同形のソース → **Issues警告0件**、`軸: ビュー(視点)`、L1/L2描画OK(`/tmp/fp_kenny_L*.png`)。
  真陽性は維持: `%%L1: 理想の姿` 単独 → FR-7.11警告あり / `%%L1: 理想の姿`+`%%L2` → FR-7.11警告あり /
  マーカーより前に `flowchart LR` 本文 → 従来どおり警告あり。

### 副産物: frontmatterの前にコメントを置くと落ちる(Mermaid本体の規則)

検証中に見つけた別の罠。マーカーの直後にコメントを書き、その次に `---` を置くと、
Mermaidがfrontmatterと認識せず**そのレベル全体が失敗する**:
...
sequenceDiagram
```
→ `Parse error on line 1: ---title: 令9期の是正経緯 … Expecting 'SPACE','NEWLINE','SD', got '-'`
